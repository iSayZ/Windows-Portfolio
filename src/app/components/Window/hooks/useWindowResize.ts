...
export const useWindowResize = (
  ...
  const handleResizeMove = useCallback((e: MouseEvent) => {
    ...
    if (direction.includes('w')) {
      ...
      const adjustedDeltaX = Math.max(Math.min(deltaX, maxLeftDelta), -resizeState.initialLeft);
    ...
    if (direction.includes('n')) {
      ...
      const adjustedDeltaY = Math.max(Math.min(deltaY, maxTopDelta), -resizeState.initialTop);
